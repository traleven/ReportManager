...
namespace ReportManager
{
	...
	public class Form1 : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
			...
			this.printPreviewDialog1.Location = new System.Drawing.Point(667, 18);
		...
		private void menuItem1_Click(object sender, System.EventArgs e)
		{
				...
				/*foreach(Band b in activeReport.Bands)
					foreach(Control c in b.Controls)
						if(c is Controls.SMTextBox)
							(c as Controls.SMTextBox).PrepareToPrint();
				*/
			{
			/*	Word.Application app;
				...
				Word.Table table1 = doc.Tables.Add(
					doc.Range(ref zero, ref zero),
					activeReport.Bands.Count, 1, ref mis, ref mis);

				int cellNumber = 1;
				for (int rowCount = 1; rowCount <= table1.Rows.Count;
					rowCount++)
				{
					object TRUE = true, ONE = 1, COLS = activeReport.Bands[rowCount-1].Controls.Count;
					bW = activeReport.Bands[rowCount-1].Width;
					pW = table1.Rows[rowCount].Cells[1].Width;
					k = pW/bW;
					table1.Rows[rowCount].Cells[1].Split(ref ONE, ref COLS);
					SortedList lst = new SortedList();
					foreach(Control c in activeReport.Bands[rowCount-1].Controls)
					{
						lst.Add(c.Left,c);
					}
					for (int columnCount = 1; columnCount <= lst.Count; columnCount++)
					{
						Word.Cell cell;
						cell = table1.Rows[rowCount].Cells[columnCount];
						if(columnCount<lst.Count)
							cell.Width = (float)k*(((Control)lst.GetByIndex(columnCount)).Left-((Control)lst.GetByIndex(columnCount-1)).Left);
						else
							cell.Width = (float)k*(activeReport.Bands[rowCount-1].Width-((Control)lst.GetByIndex(columnCount-1)).Left);
						cell.Range.Font = 
							FontToWord(((Control)lst.GetByIndex(columnCount-1)).Font);
						cell.Range.ParagraphFormat.Alignment = 
							getAlign((Control)lst.GetByIndex(columnCount-1));
						cell.Range.Text =
							((Control)lst.GetByIndex(columnCount-1)).Text;
						//activeReport.Bands[rowCount-1].ToString();
						//cellNumber.ToString();
						cellNumber++;
					}
				}
				...
				mis = Type.Missing;
				app.Quit(ref mis, ref mis, ref mis);
		...
		/*private Word.Font FontToWord(System.Drawing.Font font)
		{
			Word.Font res = new Word.FontClass();
			res.Bold = font.Bold?1:0;
			res.Italic = font.Italic?1:0;
			res.Name = font.Name;
			res.Size = font.Size-1;
			//res.Underline = font.Underline?Word.WdUnderline;
			return res;
		}

		private Word.WdParagraphAlignment getAlign(Control c)
		{
			Word.WdParagraphAlignment ret = Word.WdParagraphAlignment.wdAlignParagraphLeft;
			if(c is Controls.SMTextBox)
			{
				//if((c as Controls.SMTextBox);
			} 
			else if(c is Controls.STextBox)
			{
				HorizontalAlignment al = (c as Controls.STextBox).TextAlign;
				if(al==HorizontalAlignment.Center)
					ret = Word.WdParagraphAlignment.wdAlignParagraphCenter;
				if(al==HorizontalAlignment.Left)
					ret = Word.WdParagraphAlignment.wdAlignParagraphLeft;
				if(al==HorizontalAlignment.Right)
					ret = Word.WdParagraphAlignment.wdAlignParagraphRight;
			}
			else if(c is Controls.SLabel)
			{
				ContentAlignment al = (c as Controls.SLabel).TextAlign;
				int all = (int)al;
				al = ContentAlignment.TopCenter;
				all = (int)al;
				if(al==ContentAlignment.BottomCenter)
					ret = Word.WdParagraphAlignment.wdAlignParagraphCenter;
				if(al==ContentAlignment.MiddleCenter)
					ret = Word.WdParagraphAlignment.wdAlignParagraphCenter;
				if(al==ContentAlignment.TopCenter)
					ret = Word.WdParagraphAlignment.wdAlignParagraphCenter;

				if(al==ContentAlignment.BottomLeft)
					ret = Word.WdParagraphAlignment.wdAlignParagraphLeft;
				if(al==ContentAlignment.MiddleLeft)
					ret = Word.WdParagraphAlignment.wdAlignParagraphLeft;
				if(al==ContentAlignment.TopLeft)
					ret = Word.WdParagraphAlignment.wdAlignParagraphLeft;

				if(al==ContentAlignment.BottomRight)
					ret = Word.WdParagraphAlignment.wdAlignParagraphRight;
				if(al==ContentAlignment.MiddleRight)
					ret = Word.WdParagraphAlignment.wdAlignParagraphRight;
				if(al==ContentAlignment.TopRight)
					ret = Word.WdParagraphAlignment.wdAlignParagraphRight;
			}
			return ret;
		}*/
